...
public class StringCombination {
	...
	//方法一：元素i是否在子集dest中
	public static void combination(int i, ArrayList<String> src, ArrayList<String> dest) {
		if(i >= src.size()) {//退出条件：对最后一个元素的处理完
			System.out.println(dest.toString());
		} else {
			dest.add(src.get(i)); //子集包含元素i
			combination(i+1, src, dest); 
	...
	//方法二
	private static void combination(char[] a){
		int n = a.length;
