 ...
 * 
 * 本题的变形：如何在O(1)时间复杂度内获取一个队列的最大值。
 * （去掉窗口，也是利用辅助双端队列由大到小存放元素脚标）
 * （还有一种方式就是：先用面试题21的方法封装一个能直接获取最大值的自定义栈对象，然后使用剑指offer面试题7的方式，用两个自定义栈来模拟
 *                    实现队列，返回两个自定义栈中最大值的较大者。这是纯栈方式）
 * 
 * @author liangjian
 * @since 2016-01-29
 *
...
public class MaxWindowArray {
	 ...
	 * 
	 * 为什么使用双端队列（双向链表），因为我要获取队首元素，还要往队尾添加元素。
	 * 
	 */
